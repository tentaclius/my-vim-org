...
function myOrgFold(lnum)
   ...
   -- if this is the last line or the next line is a header, then lower the level
   local nextline = getline(lnum + 1)
   if vim.api.nvim_buf_line_count(0) == lnum or nextline:match("^[*]+ ") then
      return "s1"
   end
